...
class ExpectedSarsaAgent:
    ...
    def __init__(self, actions=0, alpha=0.5, gamma=0.99, random_seed=0):
        """
        The Q-values will be stored in a dictionary. Each key will be of the
        format: ((x, y), a).params:
    ...
    def learn(self, state, action, reward, next_state, epsilon):
        ...
        next_state_probs = self.action_probs(next_state, epsilon)
        # Probability for taking each action in next state
        q_next_state = [
            self.get_Q_value(next_state, action) for action in self.actions]
        ...
        if q_current is None:
            ...
            self.Q[(state, action)] = q_current +\
                 (self.alpha * (
                     reward + self.gamma * next_state_expectation - q_current))
    ...
    def greedy_action_selection(self, state):
        ...
        maxQ = max(q_values)
        # There might be cases where there are multiple actions with the same
        # high q_value. Choose randomly then
        count_maxQ = q_values.count(maxQ)
        if count_maxQ > 1:
            # Get all the actions with the maxQ
            best_action_indexes = [
                i for i in range(len(self.actions)) if q_values[i] == maxQ]
