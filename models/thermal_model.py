...
class Thermal_model:
    ...
    def __init__(self):
        ...
        """
        thermal_data = np.genfromtxt(
            'data/ThermalData.csv', delimiter=';', skip_header=1)
    ...
    def thermal_balance_update(self, time_step, New_Mass, AC_Balance):
        ...
        thermalBalance = self.solar[self.Time_Step]*windshield_area +\
            ...
            self.new_mass*self.Cp_air*(
                 self.temperature_outside[self.Time_Step] -
    ...
    def dew_point_confort(self):
        ...
        if self.new_mass > 0:
            ...
            self.humidity_inside = self.humidity_outside[self.Time_Step]*(
                self.new_mass/self.air_mass_in_car) +\
        ...
        if self.dew_point < 10:
            # (<10°C) a bit dry for some
            confort = 1
        elif self.dew_point >= 10 and self.dew_point < 16:
            # (10 - 16°C) comfortable
            confort = 2
        elif self.dew_point >= 16 and self.dew_point < 18:
            # (16 - 18°C) getting sticky
            confort = 1
        elif self.dew_point >= 18 and self.dew_point < 21:
            # (18 - 21°C) unpleasant
            confort = 0
